...
public interface BoardRepository extends JpaRepository<Board, Long> {
    ...
    @Query("SELECT DISTINCT b FROM Board b LEFT JOIN FETCH b.tag_boardList tb left JOIN FETCH tb.locationTag tbl WHERE b.users.id = :hostId order by b.createdAt desc")
    List<Board> getBoardListByHostIdWithFetch(@Param("hostId") Long hostId);

    @Query("SELECT DISTINCT b FROM Board b LEFT JOIN FETCH b.tag_boardList tb left JOIN FETCH tb.locationTag tbl WHERE b.users.role = :role order by b.createdAt desc")
    ...
    @Query("SELECT DISTINCT b FROM Board b LEFT JOIN FETCH b.tag_boardList tb left JOIN FETCH tb.locationTag tbl WHERE b.id = :boardId")
